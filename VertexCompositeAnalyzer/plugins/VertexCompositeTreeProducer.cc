...
VertexCompositeTreeProducer::VertexCompositeTreeProducer(const edm::ParameterSet& iConfig)
{
	...
	patCompositeCandidateCollection_Token_ = consumes<pat::CompositeCandidateCollection>(iConfig.getUntrackedParameter<edm::InputTag>("patCompositeCandidates"));
...
VertexCompositeTreeProducer::fillRECO(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	for(unsigned it=0; it<v0candidates_->size(); ++it){
		...
		secvz = trk.userFloat("vertexZ"); secvx = trk.userFloat("vertexX"); secvy = trk.userFloat("vertexY");
